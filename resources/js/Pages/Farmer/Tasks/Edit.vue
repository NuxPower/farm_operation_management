<template>
  <div class="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 py-10 px-4 sm:px-6 lg:px-8">
    ...
    <div v-else class="w-full mx-auto space-y-8">
...
const plantings = computed(() => {
  // For editing, show current planting even if it's harvested/cancelled, or stick to active filter?
  // Usually better to include the current one if it's not active anymore.
  return (farmStore.plantings || []).filter(p => {
    return (p.status !== 'harvested' && p.status !== 'failed' && p.status !== 'cancelled') || p.id == form.planting_id
  })
})
...
const submitTask = async () => {
  ...
  if (!isValid) {
    ...
    await farmStore.updateTask(route.params.id, payload)
    router.push('/tasks')
  } catch (error) {
...
onMounted(async () => {
  ...
  try {
     ...
     form.field_id = task.field_id || task.planting?.field_id
     form.planting_id = task.planting_id
     form.due_date = formatDateForInput(task.due_date)
     form.description = task.description
     form.assignment_type = task.laborer_group_id ? 'group' : 'individual'
     form.assigned_to = task.assigned_to
     form.laborer_group_id = task.laborer_group_id
     form.payment_type = task.payment_type || 'wage'
     form.revenue_share_percentage = task.revenue_share_percentage
     form.wage_amount = task.wage_amount
     form.quantity = task.quantity
     form.unit = task.unit || 'bundles'
     form.unit_price = task.unit_price
